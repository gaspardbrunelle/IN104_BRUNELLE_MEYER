...
// Affiche une ligne de _____________
void print_ligne()
{
	printf("\n");
...
void afficher_erreur(int *erreur)
{
	...
	for(int k=0;k<nombre_erreur;++k){
		int i=erreur[2*k];
		int j=erreur[2*k+1];
		printf("erreur n°%d : ligne:%d , colonne:%d \n",k+1,i,j);
	printf("\n");
	}	
...
void jouer(int **M)
{
	...
	while(fini==0){
		printf("Avez-vous fini? Tapez 1 pour oui ou 0 pour non: ");
		scanf("%d",&fini);
		if(fini==0){
		int ligne;
		int colonne;
...
void copie(int M_cible[9][9],int **M_acopier)
{
...
int main()
{
	...
	copie(S,M); // on copie la matrice complète 
